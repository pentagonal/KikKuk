...
class Database
{
    ...
    public function __construct(array $configs)
    {
        ...
        if ($this->currentSelectedDriver == 'pdo_sqlite' && !isset($this->currentUserParams['path'])) {
            if (!isset($this->currentUserParams['dbname']) || !is_string($this->currentUserParams['dbname'])) {
                throw new DBALException('SQLite database path must be not empty.', E_USER_ERROR);
            }
            $this->currentUserParams['path'] = $this->currentUserParams['dbname'];
        }

        /**
         * create new parameters
         */
        $connectionParams = $this->currentUserParams;
        $connectionParams['driver'] = $this->currentSelectedDriver;

        /**
         * Unset Driver Prefix
         */
        unset($connectionParams['prefix']);

        /**
         * Create New Connection
         */
        $this->currentConnection = DriverManager::getConnection($connectionParams);
