 ...
 * -------------------- List Methods ---------------------
 *
 * @method static bool          contains(string $id)
 * @method bool          delete(string $id)
 * @method bool          deleteAll()
 * @method bool          doContains(string $id)
 * @method bool          doDelete(string $id)
 * @method false|mixed   doFetch(string $id)
 * @method array|mixed[] doFetchMultiple(array|string[] $keys)
 * @method bool          doFlush()
 * @method bool          doSave(string $id, mixed $data, int $lifetime = 0)
 * @method array|null    doGetStats()
 * @method mixed         fetch(string $id)
 * @method array|mixed[] fetchMultiple(array|string $keys)
            ...
            case self::SQLITE3:
                ...
                if (!$this->resource) {
                    ...
                    if (!$sqlite) {
                        ...
                        $config['flags'] = isset($config['flags'])
                            ? $config['flags']
                            : null;
                        $config['password'] = isset($config['password'])
                            ? $config['password']
                            : null;
                        $sqlite = new \SQLite3($config['dbname'], $config['flags'], $config['password']);
                    }
                    $this->resource = new $this->config['driver']($sqlite);
